        	...
        	steps {
					...
					container_Up = false
                    			timeout(time: 1, unit: "HOURS") { 
                        			sh "curl -u admin:admin -X GET -H \"Accept: application/json\" http://104.248.169.167:9000/api/qualitygates/project_status?projectKey=com.mycompany:demo-sapi > status.json"
    ...
    //     }
	 stage('Build') {
        ...
        stage('Deploy to Cloudhub'){
        	steps {
				 withCredentials([file(credentialsId: 'settings', variable: 'settings')]){
        	    	sh "mvn -f pom.xml -s $settings package deploy -DskipTests -Dusername=njcistika2021 -Dpassword=njcIstika2021 -DapplicationName=demo-sapi -Denvironment=dev -DmuleDeploy -Dcloudhub.region=us-east-2 -Danypoint.platform.client_id=undefined -Danypoint.platform.client_secret=undefined -Dsecure.key=mule"
		...
		// 	steps{
		// 		sh "curl --location --request POST 'http://46.101.7.76:8087/logs'  --header 'Content-Type: application/json' --data-raw '{\"username\": \"njcistika2021\",\"password\": \"njcIstika2021\",\"orgName\": \"NJC POC\",\"envName\": \"Sandbox\", \"appNames\": [\"demo-sapi\"]}'"
		// 	}
		// }	
